...
class CustomClassifier(PreTrainedModel):
    ...
    def __init__(self, config): #생성자(클래스가 객체화 될때 가장먼저 실행)
        ...
        self.custom_head = nn.Sequential(
            ...
            nn.ReLU(), # 활성함수
            nn.Dropout(0.3), # 30% 무작위 끄기(전교 1등 재우기)
...
print(inputs)
# 검증모드로 변경
model.eval() # 학습 과정은 필요없이 추론 결과만 보고자 할때
...
[-0.1598, -0.2437]
"""
# dim=1|-1 : 가로 방향으로 연산, dim=0 : 세로 방향으로 연산
