...
# 2. text -> review 컬러명 변경
ds = ds.rename_column('text', 'review')

# 3. 10자 미만의 리뷰는 거른다.
# (item) => return len(item['review']) >= 10
ds.filter(lambda item: len(item['review']) >= 10)
...
# HF 에 업로드
# hf auth login --force
REPO_ID = 'dev-Hwang/upload_test_ds'
