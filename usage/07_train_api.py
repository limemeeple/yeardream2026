...
def main():
    # 1. 데이터셋 불러오기
    dataset = load_dataset('stanfordnlp/imdb', split={"train":"train[:2000]", "test":"test[:500]"})
    ...
    args = TrainingArguments(
        ...
        # 2. Mixed perecision 설정(GPU 사양에 맞춰서 설정)
        fp16=True, # T4, A100 등 이전세대 GPU
        bf16=True, # A100, RTX3000/4000 시리즈 이상
        # compile 을 이용한 최적화
        torch_compile=True,
        # 4. Gradient Exploading 방지를 위한 cliping
        max_grad_norm=1.0,
        # 5. Gradient Accumulation 으로 큰 배치 효과 구현
...
# 메인 프로세서(스레드) 만 진입해서 실행해라(멀티 스레드 환경에서만 사용)
